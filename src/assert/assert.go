...
func (as *Assert) reportError(cd int, format string, a ...interface{}) {
	...
	as.teamCity.reportError(file, line, msg)
}

func (as *Assert) reportSuccess(cd int, format string, a ...interface{}) {
	_, file, line, ok := runtime.Caller(cd)
	if ok {
		file, _ = relativePath(file)
	} else {
	...
	msg := fmt.Sprintf(format, a...)
	as.t.Logf("%v:%v: %v", file, line, msg)
	as.teamCity.reportSuccess(file, line)
}
